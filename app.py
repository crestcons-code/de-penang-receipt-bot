...
st.set_page_config(page_title="DE Penang Autocount Donation Receipts Apps", page_icon="🏦", layout="wide")

# ── GL code options for dropdown
GL_OPTIONS = {f"{code}  {desc}": code for code, desc, _ in DONATION_MAP}

# Reverse lookup: GL code → short description
...
def render_review_and_post(rows: list, skipped_count: int = 0):
    ...
    with btn_col:
        b1, b2 = st.columns(2)
        if b1.button("✔", help="Tick All", use_container_width=True):
            st.session_state.post_all = True
            st.rerun()
        if b2.button("✘", help="Untick All", use_container_width=True):
...
with tab_dana:
    ...
    if dana_file:
        ...
        for p in posted:
            ...
            # Add base number: OR-2606153-1 → also add OR-2606153
            import re as _re
...
**Colour guide:** 🟢 Green = Found &nbsp;&nbsp; 🔴 Red = Missing
            """)
