...
# Record of each posting batch, so a volunteer can come back later - tomorrow, or
# from another PC - and pull up exactly which ORs went in together, ready to print
# from Autocount. Autocount's own API cannot print receipts, so the app's job is
# to make the batch easy to find again rather than to reproduce the document.
OR_BATCH_TAB = "_or_batches"
_OR_BATCH_HEADERS = ["Batch", "Posted at", "Posted by", "OR Number", "Date",
                     "Donor", "Amount", "GL", "Description", "WhatsApp"]


def save_or_batch(client: gspread.Client, spreadsheet_id: str, batch_id: str,
                  posted_by: str, rows: list) -> None:
    """
    Append one posting batch. rows: list of dicts with keys
    or_number, date, donor, amount, gl, description, whatsapp.
    Appends only - a batch already written is never rewritten.
    """
    if not rows:
        return
    import datetime as _dt
    sh = client.open_by_key(spreadsheet_id)
    try:
        ws = sh.worksheet(OR_BATCH_TAB)
    except gspread.WorksheetNotFound:
        ws = sh.add_worksheet(title=OR_BATCH_TAB, rows=2000, cols=len(_OR_BATCH_HEADERS))
        ws.update([_OR_BATCH_HEADERS], value_input_option="USER_ENTERED")

    stamp = _dt.datetime.now().strftime("%Y-%m-%d %H:%M")
    out = [[batch_id, stamp, posted_by, r.get("or_number", ""), r.get("date", ""),
            r.get("donor", ""), r.get("amount", ""), r.get("gl", ""),
            r.get("description", ""), r.get("whatsapp", "")] for r in rows]
    ws.append_rows(out, value_input_option="USER_ENTERED")


def list_or_batches(client: gspread.Client, spreadsheet_id: str) -> list:
    """
    Return saved batches, newest first:
    [{"batch", "posted_at", "posted_by", "count", "or_numbers": [...]}, ...]
    """
    sh = client.open_by_key(spreadsheet_id)
    try:
        ws = sh.worksheet(OR_BATCH_TAB)
    ...
    batches = {}
    for row in values[1:]:
        row = row + [""] * (len(_OR_BATCH_HEADERS) - len(row))
        bid = row[0].strip()
        if not bid:
            continue
        b = batches.setdefault(bid, {"batch": bid, "posted_at": row[1].strip(),
                                     "posted_by": row[2].strip(), "or_numbers": []})
        if row[3].strip():
            b["or_numbers"].append(row[3].strip())
    out = list(batches.values())
    for b in out:
        b["count"] = len(b["or_numbers"])
    out.sort(key=lambda b: b["posted_at"], reverse=True)
    return out
